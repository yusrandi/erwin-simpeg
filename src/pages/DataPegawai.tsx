...
export default function DataPegawai() {
  ...
  const units = [...new Set(data.map(d => d.unit_kerja))].sort()

  async function fetchData() {
    setLoading(true)
    const { data: rows, error } = await supabase.from("pegawai").select("*").order("id", { ascending: true })
    if (error) { toast({ title: "Gagal memuat data", variant: "destructive" }); setLoading(false); return }
    setData(rows ?? [])
    setLoading(false)
  }
  ...
  useEffect(() => {
    ...
    if (filterUnit !== "SEMUA") result = result.filter(d => d.unit_kerja === filterUnit)
    setFiltered(result)
    setPage(1)
  ...
  async function handleEdit(formData: PegawaiFormData) {
    ...
    setSaving(true)
    const { error } = await supabase.from("pegawai").update(formData).eq("id", editTarget.id)
    setSaving(false)
    if (error) { toast({ title: "Gagal menyimpan", description: error.message, variant: "destructive" }); return }
    toast({ title: "Berhasil disimpan", variant: "success" as never })
    setEditTarget(null)
    fetchData()
  }
  ...
  async function handleDelete() {
    ...
    setSaving(true)
    const { error } = await supabase.from("pegawai").delete().eq("id", deleteTarget.id)
    setSaving(false)
    if (error) { toast({ title: "Gagal menghapus", description: error.message, variant: "destructive" }); return }
    toast({ title: "Data dihapus", variant: "success" as never })
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{filtered.length} dari {data.length} pegawai ditampilkan</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input className="pl-9" placeholder="Cari nama, NIP, jabatan..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        <Select value={filterUnit} onValueChange={setFilterUnit}>
          <SelectTrigger className="w-36"><SelectValue placeholder="Unit Kerja" /></SelectTrigger>
          <SelectContent>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={p.id}>
                ...
                <TableCell className="font-medium text-slate-800">{p.nama_pegawai}</TableCell>
                <TableCell className="text-xs text-slate-500">{p.id_nip}</TableCell>
                <TableCell className="text-xs">{p.pendidikan_terakhir}</TableCell>
                <TableCell><Badge variant="outline" className="text-xs">{p.unit_kerja}</Badge></TableCell>
                ...
                <TableCell className="text-xs text-slate-500">{p.jalur_masuk}</TableCell>
                <TableCell className="text-right">
                  <div className="flex gap-1 justify-end">
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setEditTarget(p)}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((p, idx, arr) => (
                <>
                  {idx > 0 && arr[idx - 1] !== p - 1 && <span key={`dots-${p}`} className="px-2 text-muted-foreground self-center">…</span>}
      ...
      <Dialog open={!!editTarget} onOpenChange={open => !open && setEditTarget(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{editTarget?.nama_pegawai} · NIP {editTarget?.id_nip}</DialogDescription>
          </DialogHeader>
          {editTarget && (
            <PegawaiForm
      ...
      <Dialog open={!!deleteTarget} onOpenChange={open => !open && setDeleteTarget(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Apakah Anda yakin ingin menghapus data <strong>{deleteTarget?.nama_pegawai}</strong>?
