...
export default function LabaRugi() {
  ...
  async function fetchData() {
    ...
    // 1. Ambil semua akun PENDAPATAN & BEBAN dari master
    ...
    if (!akunList || akunList.length === 0) { setRows([]); setLoading(false); return }

    // 2. Ambil jurnal_detail untuk akun tersebut
    const kodeList = akunList.map(a => a.kode)
    ...
    // 4. Hitung total per akun
    const map = new Map<string, number>()
    filtered.forEach(d => {
      const akun = akunList.find(a => a.kode === d.akun_kode)
      if (!akun) return
      // Pendapatan tipe normal KREDIT: total = kredit - debit
      // Beban tipe normal DEBIT: total = debit - kredit
    ...
    // 5. Build rows
    const result: LabaRugiRow[] = akunList.map(a => ({
      akun_kode: a.kode,
      akun_nama: a.nama,
  ...
  const totalBeban = rows
    .filter(r => r.kelompok === "BEBAN")
    .reduce((s, r) => s + r.total, 0)
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Menampilkan seluruh data pendapatan dan beban
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {rows.filter(r => r.kelompok === "PENDAPATAN").length} akun
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {rows.filter(r => r.kelompok === "BEBAN").length} akun
            </p>
          </CardContent>
        </Card>
        <Card className={isLaba
          ...
          <CardContent className="p-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {isLaba ? "Pendapatan > Beban" : "Beban > Pendapatan"}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 text-muted-foreground text-sm border rounded-lg border-dashed gap-2">
          ...
          <p>Tambahkan akun di halaman <strong>Master Akun</strong> dengan kelompok PENDAPATAN atau BEBAN.</p>
        </div>
      ) : (
        <Card>
            ...
            <Table>
              ...
              <TableBody>
                ...
                <TableRow className="bg-muted/50 border-t-2">
                  <TableCell colSpan={2} className="font-bold py-3">
