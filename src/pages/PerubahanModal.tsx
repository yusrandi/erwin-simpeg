...
export default function PerubahanModal() {
  ...
  const [periodeAwal, setPeriodeAwal] = useState(
    new Date(new Date().getFullYear(), 0, 1).toISOString().slice(0, 10) // 1 Jan tahun ini
  )
  ...
  async function fetchData() {
    ...
    const { data: akunModal } = await supabase
      .from("master_akun")
      .select("kode, nama, tipe_normal")
      .eq("kelompok", "MODAL")
      .order("kode")

    // 2. Ambil akun PENDAPATAN & BEBAN
    ...
    if (semuaKode.length === 0) { setRows([]); setLoading(false); return }

    // 3. Ambil semua jurnal detail
    ...
    // Helper: hitung saldo akun sampai tanggal tertentu
    ...
    // Helper: hitung laba rugi sampai tanggal tertentu
    ...
    // 4. Hitung saldo awal (sebelum periodeAwal) & akhir (s.d. periodeAkhir)
  ...
  useEffect(() => { fetchData() }, [periodeAwal, periodeAkhir])
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Informasi nilai modal selama periode tertentu
          </p>
        </div>
          ...
          <Card className={totalPerubahan >= 0
            ...
            <CardContent className="p-5">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {totalPerubahan >= 0 ? "Modal bertambah" : "Modal berkurang"}
              </p>
            </CardContent>
          </Card>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 text-muted-foreground text-sm border rounded-lg border-dashed gap-2">
          ...
          <p>Tambahkan di halaman <strong>Master Akun</strong> dengan kelompok MODAL.</p>
        </div>
      ) : (
        <Card>
