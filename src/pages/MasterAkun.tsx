...
// Tipe normal default per kelompok
...
const emptyForm = (): MasterAkun => ({
  kode: "", nama: "", kelompok: "ASET", tipe_normal: "DEBIT"
})
...
export default function MasterAkunPage() {
  ...
  const [form, setForm] = useState<MasterAkun>(emptyForm())
  ...
  // Auto set tipe_normal saat kelompok berubah
  function handleKelompokChange(val: KelompokAkun) {
    setForm(p => ({ ...p, kelompok: val, tipe_normal: defaultTipe[val] }))
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editTarget) {
      ...
      const { error } = await supabase.from("master_akun").insert(form)
      if (error) {
        toast({ title: "Gagal menyimpan", description: error.message, variant: "destructive" })
        setSaving(false); return
      }
  ...
  async function handleDelete() {
    ...
    setSaving(true)
    const { error } = await supabase.from("master_akun").delete().eq("kode", deleteTarget.kode)
    setSaving(false)
    if (error) {
      toast({ title: "Gagal menghapus", description: "Akun mungkin masih digunakan di jurnal.", variant: "destructive" })
  ...
  function openEdit(akun: MasterAkun) {
    setEditTarget(akun)
    setForm({ ...akun })
    setOpenForm(true)
  }
  ...
  const countPerKelompok = (k: KelompokAkun) => data.filter(d => d.kelompok === k).length
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Daftar akun sebagai dasar seluruh transaksi keuangan</p>
        </div>
      ...
      {/* Summary per kelompok */}
      <div className="grid grid-cols-5 gap-3">
        {KELOMPOK.map(k => (
          <Card
            key={k}
            className={`cursor-pointer border transition-all ${filterKelompok === k ? "ring-2 ring-foreground" : ""}`}
            onClick={() => setFilterKelompok(prev => prev === k ? "SEMUA" : k)}
          >
            <CardContent className="p-4">
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${kelompokBadge[k]}`}>{k}</span>
              <p className="text-2xl font-bold text-foreground mt-2">{countPerKelompok(k)}</p>
      ...
      <div className="flex gap-3">
        ...
        <p className="text-sm text-muted-foreground self-center">
          {filtered.length} dari {data.length} akun
        </p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={akun.kode}>
                <TableCell className="font-mono font-semibold text-sm">{akun.kode}</TableCell>
                <TableCell className="text-sm">{akun.nama}</TableCell>
                <TableCell>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${kelompokBadge[akun.kelompok]}`}>
                    {akun.kelompok}
                  </span>
                </TableCell>
                <TableCell>
                  <Badge variant={akun.tipe_normal === "DEBIT" ? "default" : "secondary"} className="text-xs">
                    {akun.tipe_normal}
                  </Badge>
                </TableCell>
                <TableCell className="text-right">
                  <div className="flex gap-1 justify-end">
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => openEdit(akun)}>
      ...
      <Dialog open={openForm} onOpenChange={open => { if (!open) { setOpenForm(false); setEditTarget(null) } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editTarget ? "Edit Akun" : "Tambah Akun Baru"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
              />
              {editTarget && <p className="text-xs text-muted-foreground">Kode akun tidak bisa diubah</p>}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={form.kelompok} onValueChange={v => handleKelompokChange(v as KelompokAkun)}>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Otomatis terisi sesuai kelompok. Ubah hanya jika diperlukan.
              </p>
            </div>
            <div className="flex gap-3 pt-1">
              ...
              <Button type="button" variant="outline" onClick={() => { setOpenForm(false); setEditTarget(null) }}>
                Batal
              </Button>
            </div>
          </form>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={open => !open && setDeleteTarget(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <br /><br />
            <span className="text-destructive text-xs">⚠ Akun yang sudah dipakai di jurnal tidak bisa dihapus.</span>
          </p>
